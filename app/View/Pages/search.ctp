...
<div class="search-boundary">
	<?php
		if(!empty($search_results)){
					...
					if($data['PageContent']['content']){
						if($cleaned){
							if($root == 'projects'){
								...
								$shortened_text = strlen($text) >= 150 ? substr($text, 0, 150) . ' ...' : $text;
			...
			echo '<div class="large-12 columns text-right">'.$this->Html->link(__(('view all search'), true), array('controller'=> 'pages', 'action'=>'more-search', $keywords), array('escape' => false, 'class'=>'read-more-i')).'</div>';
