...
<?php 
	...
	foreach($projects as $project):
		...
		$j++;	
	endforeach;
	echo '<li class="ex-title" onclick="location.href=\''.$this->Html->url(DS.'pages'.DS.'projects', true).'\'"><a href="#">All</a></li>';
	echo '</ul>';
	...
	foreach($projects as $project):
		if($k == 1){
			...
			echo $this->Html->link(__(('read more'), true), array('controller'=> 'pages', 'action'=>'projects#'.$project['PageContent']['slug']), array('escape' => false, 'class'=>'read-more'));
			echo '</div></div></div>';
		}else{
